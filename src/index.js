...
function Drawer(canvas) {
  ...
  this.shadow = function shadow(option) {
  ...
  this.rect = function rect(option) {
  ...
  this.arc = function arc(option) {
    const r = random(100, Math.min(option.width, option.height) / 2);
    ctx.arc(0, 0, r, 0, 2 * Math.PI);
  };

  this.draw = function draw(sharp, option) {
    ctx.beginPath();
    this.shadow({
      blur: 20,
    });
  ...
  this.fill = function fill(color, width, height) {
    ctx.fillStyle = color;
...
function MaterialImage(config = {}) {
  ...
  const element = el || document.querySelector('body');
...
MaterialImage.prototype.protract = function protract() {
  ...
  if (!this.debug) {
    ...
    setTimeout(() => {
      ...
      if (i <= count) setTimeout(protract, 1000);
    }, 1000);
  }
};

MaterialImage.prototype.adjust = function adjust() {
